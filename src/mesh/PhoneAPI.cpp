...
// Date: 08/12/2021
// Remarks: Indirectly reset controller by receiving startConfig command from phone.
void PhoneAPI::resetController()
{
    DEBUG_MSG("[resetController]: Prepare REBOOTING device!\n");
    
    // Indirectly call observe() function for the seconds time will caused
    // error and reboot the device 
...
void PhoneAPI::handleStartConfig()
{
    ...
    if (!isConnected()) {
        DEBUG_MSG("[handleStartConfig]: SET PHONE connected\n");
...
{
    DEBUG_MSG("State [handleToRadio]: ENTER function\n");
    ...
    if (pb_decode_from_bytes(buf, bufLength, ToRadio_fields, &toRadioScratch)) {
        ...
        case ToRadio_packet_tag:
            DEBUG_MSG("State [handleToRadio]: ToRadio_packet_tag\n");
            
            return handleToRadioPacket(toRadioScratch.packet);

        case ToRadio_want_config_id_tag:
            DEBUG_MSG("State [handleToRadio]: ToRadio_want_config_id_tag\n");
        ...
        case ToRadio_disconnect_tag:
            DEBUG_MSG("State [handleToRadio]: ToRadio_disconnect_tag\n");
...
size_t PhoneAPI::getFromRadio(uint8_t *buf)
{
    ...
    case STATE_SEND_MY_INFO:
        DEBUG_MSG("State [getFromRadio]: STATE_SEND_MY_INFO\n");
    ...
    case STATE_SEND_NODEINFO: {
        DEBUG_MSG("State [getFromRadio]: STATE_SEND_NODEINFO\n");
    ...
    case STATE_SEND_COMPLETE_ID:
        DEBUG_MSG("State [getFromRadio]: STATE_SEND_COMPLETE_ID\n");
    ...
    case STATE_SEND_PACKETS:
        DEBUG_MSG("State [getFromRadio]: STATE_SEND_PACKETS\n");
...
bool PhoneAPI::available()
{
    switch (state) {
    case STATE_SEND_NOTHING:
        DEBUG_MSG("State [available]: STATE_SEND_NOTHING\n");

        return false;

    case STATE_SEND_MY_INFO:
        DEBUG_MSG("State [available]: STATE_SEND_MY_INFO\n");
    ...
    case STATE_SEND_COMPLETE_ID:
        DEBUG_MSG("State [available]: STATE_SEND_COMPLETE_ID\n");

        return true;

    case STATE_SEND_PACKETS: {
        DEBUG_MSG("State [available]: STATE_SEND_PACKETS\n");
